...
H55_MagiHutsQty = table.length(H55_MagiHuts);
...
H55_BankActions = {}; for i=1,8 do H55_BankActions[i] = 0 end;
H55_BankLastVisit = {};
...
H55_Stables = GetObjectNamesByType("BUILDING_STABLES");
H55_StablesQty = table.length(H55_Stables);
...
-- H55_ManaObjectUsers = {};
-- for i,fofy in H55_FOfYouth do
	-- H55_ManaObjectUsers[fofy] = {};
-- end;
-- for i,oasis in H55_Oasae do
	-- H55_ManaObjectUsers[oasis] = {};
-- end;
-- for i,faerie in H55_Faeries do
	-- H55_ManaObjectUsers[faerie] = {};
-- end;
-- for i,meshrine in H55_MEShrines do
	-- H55_ManaObjectUsers[meshrine] = {};
-- end;
-- for i,fsanct in H55_FSanctuaries do
	-- H55_ManaObjectUsers[fsanct] = {};
-- end;
-- for i,idol in H55_Idols do
	-- H55_ManaObjectUsers[idol] = {};
-- end;
-- for i,rally in H55_RallyFlags do
	-- H55_ManaObjectUsers[rally] = {};
-- end;
-- for i,stable in H55_Stables do
	-- H55_ManaObjectUsers[stable] = {};
-- end;
...
function H55_PrepareAdvMap()
	...
	if H55_EnableModes == 1 then
			...
			for i=1,8 do
				if (GetPlayerState(i) == 1) and (H55_IsThisAIPlayer(i) ~= 1) then
					...
					if choseres == 0 and chosegold == 0 then
						if heroes ~= nil then
							if HasArtefact(heroes[0],ARTIFACT_WAND_OF_X) == 1 then
								choseartifact = 1;
								somebodyartifact = 1;
								RemoveArtefact(heroes[0],ARTIFACT_WAND_OF_X);
							elseif HasArtefact(heroes[0],ARTIFACT_SCROLL_OF_SPELL_X) == 1 then
								choseartifact = 1;
								somebodyartifact = 1;
								RemoveArtefact(heroes[0],ARTIFACT_SCROLL_OF_SPELL_X);	
							else
		...
		if H55_MapType == "Scenario" then
			if H55_GameMode == 0 then
				print("H55 Player chose casual mode for scenario");
			else
				H55_GameMode = 1;
				print("H55 Force Default mode without cheating for scenario ");
			end;
		elseif 	H55_MapType == "Campaign" then
			H55_GameMode = 1;
			print("H55 Force Default mode without cheating for campaign");
		else	
			if amountai ~= 0 then
				...
				if heroes ~= nil then
					...
					if fc == 8 then
						...
						elseif hero == "Hero9" then
							H55_MakeStartArmy(hero,fc,1,20,0,0);--Kilghan
				...
				-- if heroes ~= nil then			
					-- for i,hero in heroes do
						-- if H55_GetHeroClassType(hero) == "Might" or H55_GetHeroClassType(hero) == "Chieftain" then
							-- GiveHeroSkill(hero,1);--2
							-- GiveHeroSkill(hero,1);--2
						-- elseif H55_GetHeroClassType(hero) == "Magic" then
							-- GiveHeroSkill(hero,1);--3
							-- GiveHeroSkill(hero,1);--3
						-- else
							-- GiveHeroSkill(hero,1);--6
							-- GiveHeroSkill(hero,1);--6
						-- end;				
					-- end;
				-- end;
	...
	if H55_MapType ~= "Campaign" and H55_MapType ~= "RMG" then
		if H55_ObelisksQty > 0 then
			for i,Obelisk in H55_Obelisks do
				H55_ObeliskArmies[Obelisk] = {};
				for i = 1,14 do
	...
	------------------------------------------------------------------------------------------------------------------------------------------------
	--H55_DEBUG = {111a,"HillFort",1,"NoHero"};--------------------------------------------------------------------------------------------------
	------------------------------------------------------------------------------------------------------------------------------------------------
	
	H55_TriggerToObjectType("BUILDING_HILL_FORT",OBJECT_TOUCH_TRIGGER,"H55_HillFortVisit",nil);
...
function H55_HillFortVisit(hero,building)
	local player = GetObjectOwner(hero);
	if H55_NoHillFortsOnARMG == 0 then
		Trigger(OBJECT_TOUCH_TRIGGER,building,nil);
		SetObjectEnabled(building,not nil);
		MakeHeroInteractWithObject(hero,building);
		...
		if H55_HillFortBoostVisitors[hero] == nil then
			QuestionBoxForPlayers(GetPlayerFilter(player),{"/Text/Game/Scripts/HillFortSkillBoost.txt"},
			"H55_HillFortAT('"..hero.."','"..building.."')","H55_HillFortDF('"..hero.."','"..building.."')");	
		else
			ShowFlyingSign("/Text/Game/Scripts/HillFortAlready.txt",hero,player,8);
		end;
	end;
end;

function H55_HillFortAT(hero,building)
	local player = GetObjectOwner(hero);
	H55_HillFortBoostVisitors[hero] = 1;
	ChangeHeroStat(hero,STAT_ATTACK,1);
	ShowFlyingSign("/Text/Game/Scripts/Attack.txt",hero,player,5);	
end;

function H55_HillFortDF(hero,building)
	local player = GetObjectOwner(hero);
	H55_HillFortBoostVisitors[hero] = 1;
	ChangeHeroStat(hero,STAT_DEFENSE,1);
...
function H55_SphinxVisit(hero,sphinx)
	local player = GetObjectOwner(hero);
	if H55_IsThisAIPlayer(player) ~= 1 then startThread(H55_CheckAdvMapObjQueue,player) else H55_BankActions[player] = 0 end;
	if H55_BankActions[player] == 1 then return nil end;
...
function H55_MagiHutVisit(hero,building)
	H55_EndDoorSpin[hero] = 0;
	H55_BreakDoorSpin[hero] = 0;
	local player = GetObjectOwner(hero);
	if H55_IsThisAIPlayer(player) == 1 then
		H55_MagiHutRefuseAI(hero,building);
	elseif H55_DimensionDoorsOnThisMap == 1 and H55_InterfacingStatus[hero] == 0 then
		QuestionBoxForPlayers(GetPlayerFilter(player),{"/Text/Game/Scripts/TownPortal/DD_Question.txt"},
		"H55_MagiHutAccept('"..hero.."','"..building.."')","H55_MagiHutRefuse('"..hero.."','"..building.."')");
	elseif H55_DimensionDoorsOnThisMap == 1 and H55_InterfacingStatus[hero] == 1 then
		H55_CancelDimensionDoor(hero);	
		ShowFlyingSign("/Text/Game/Scripts/TownPortal/DD_VisitEnded.txt",hero,player,5);		
	else
		MessageBoxForPlayers(GetPlayerFilter(player),"/Text/Game/Scripts/TownPortal/DD_Disabled.txt",
		"H55_MagiHutRefuse('"..hero.."','"..building.."')");
	end;
end;

function H55_MagiHutRefuse(hero,building)
	local player = GetObjectOwner(hero);
	if H55_HasTownWithGuild(player) then
		if H55_MagiHutUsers[hero] ~= H55_Day then
			local mana = GetHeroStat(hero,STAT_MANA_POINTS);
			local manatotal = H55_GetMaxMana(hero);
			local amount = 0;
			if mana < manatotal then
				amount = (manatotal-mana);
				H55_ModifyMana(hero,amount);
				ShowFlyingSign({"/Text/Game/Scripts/TownPortal/DD_Mana.txt";mana=amount},hero,player,5);
				H55_MagiHutUsers[hero] = H55_Day;
				MarkObjectAsVisited(building,hero);				
			else
				ShowFlyingSign("/Text/Game/Scripts/TownPortal/DD_FullMana.txt",hero,player,5);		
			end;
		else
			ShowFlyingSign("/Text/Game/Scripts/TownPortal/DD_Already.txt",hero,player,5);
		end;
	else
		ShowFlyingSign("/Text/Game/Scripts/TownPortal/DD_NoGuild.txt",hero,player,5);
	end;
end;

function H55_MagiHutRefuseAI(hero,building)
	Trigger(OBJECT_TOUCH_TRIGGER,building,nil);
	SetObjectEnabled(building,not nil);
	MakeHeroInteractWithObject(hero,building);
	Trigger(OBJECT_TOUCH_TRIGGER,building,"H55_MagiHutVisit");
	SetObjectEnabled(building,nil);	
	print("H55 AI visited Magi Hut!");
end;

function H55_MagiHutAccept(hero,building)
	local player = GetObjectOwner(hero);
	local towns = GetObjectNamesByType("TOWN");
	local targets = 0;
	if H55_IsEnemyNearby(hero,8)==0 then
		...
		if targets > 1 then
			for i,town in towns do
				if (player == GetObjectOwner(town)) then
					...
					if H55_BreakDoorSpin[hero] == 1 then break end;
				end;
			end;
		elseif targets == 1 then
			for i,town in towns do
				if (player == GetObjectOwner(town)) then			
					H55_InterfaceNow(hero,town);
				end;			
			end;
		else
			ShowFlyingSign("/Text/Game/Scripts/TownPortal/DD_NoTown.txt",hero,player,5);
		end;
	else
		ShowFlyingSign("/Text/Game/Scripts/TownPortal/DD_Enemy.txt",hero,player,5);
	end;
end;

function H55_UnlockNextDoor(hero)
	H55_EndDoorSpin[hero] = 1;
end;

function H55_InterfaceNow(hero,town)
	H55_BreakDoorSpin[hero] = 1;
	...
	if H55_IsAnyHeroInGateTile(town,x,y,z) == 1 then
		ShowFlyingSign("/Text/Game/Scripts/TownPortal/DD_GateFull.txt",hero,player,5);	
		H55_EndDoorSpin[hero] = 1;
	elseif H55_IsEnemyNearby(town,8)==1 then
		ShowFlyingSign("/Text/Game/Scripts/TownPortal/DD_TownEnemy.txt",hero,player,5);
		H55_EndDoorSpin[hero] = 1;
	else
		-- if movepoints < 156 then
			-- local add = (156-movepoints);
			-- ChangeHeroStat(hero,STAT_MOVE_POINTS,add);
		-- end;
		H55_InterfacingStatus[hero] = 1;
		H55_InterfacingTown[hero] = town;
		H55_InterfacingPosition[hero] = {["x"]=xx,["y"]=yy,["z"]=zz};
		H55_EndDoorSpin[hero] = 1;
...
-- function H55_ManaObjectVisit(hero,object)
	...
	-- -- if H55_IsThisAIPlayer(player) == 1 then
		...
		-- -- Trigger(OBJECT_TOUCH_TRIGGER,object,"H55_ManaObjectVisit");
	-- -- else	
		-- if H55_ManaObjectUsers[object][hero] ~= H55_Day then
			-- local mana = GetHeroStat(hero,STAT_MANA_POINTS);
			-- local manatotal = H55_GetMaxMana(hero);
			-- local amount = 10 + (2 * GetHeroLevel(hero));
			-- if mana < manatotal then		
				-- H55_ModifyMana(hero,amount);
			-- end;
			-- H55_ManaObjectUsers[object][hero] = H55_Day;
			-- if H55_IsThisAIPlayer(player) ~= 1 then 
				-- ShowFlyingSign("/Text/Game/Scripts/Duel/ManaObservatory.txt",hero,player,5);
				-- sleep(2);
			-- end;
...
function H55_SeerHutRewardA(hero,mermaid)
	...
	if choice1 == 2 then
		...
		if level < 50 then
			local levelup = H55_ExpTable[level+1] - H55_ExpTable[level];
			GiveExp(hero,levelup);
		else
			GiveExp(hero,H55_ExpTable[50]);
		end;
	end;
...
function H55_GetBankDifMultiplier()
	...
	if H55_AveragePowerRating > 2500 then
		H55_MonsterSettingAdjust = 0.7 + (((H55_AveragePowerRating-2500)/10)/1000);
	elseif H55_AveragePowerRating < 2500 then
		H55_MonsterSettingAdjust = 0.7 - (((2500-H55_AveragePowerRating)/10)/1000);
	else
		H55_MonsterSettingAdjust = 0.7;
	end;
	local reduction = H55_MonsterSettingAdjust + ((H55_BankDay/28)*0.1);
	if H55_PvPBanks == 1 then reduction = reduction + 0.1 end;
	...
	-- end;
	-- if H55_PvPBanks == 1 and H55_Difficulty == 2 then reduction = reduction + 0.05 end;
	-- if H55_PvPBanks == 1 and H55_Difficulty == 3 then reduction = reduction + 0.1 end;
...
function H55_CheckAdvMapObjQueue(player)
	if H55_BankActions[player] == 0 then
		H55_BankActions[player] = 1;
		sleep(8);
		H55_BankActions[player] = 0;
	else return end;
end;
...
function H55_UtopiaVisit(hero,bank)
	...
	if H55_GetLastVisited(bank) > 6 and H55_GetPlayerLastVisited(player,bank) > 55 then
		...
		if H55_IsThisAIPlayer(player) == 1 then
			...
			local increase = 1+(H55_BankDay/560);

			local cntef = random(4)+ math.round((increase*(multiplier*H55_NeutralCreaturesGrowth[1])));
			local cntew = random(4)+ math.round((increase*(multiplier*H55_NeutralCreaturesGrowth[2])));
			local cntee = random(4)+ math.round((increase*(multiplier*H55_NeutralCreaturesGrowth[3])));
			local cntea = random(4)+ math.round((increase*(multiplier*H55_NeutralCreaturesGrowth[4])));				

			local cnt04 = random(5)+ math.round((increase*(multiplier*5)));
			local cnt05 = random(4)+ math.round((increase*(multiplier*3)));
			local cnt06 = random(3)+ math.round((increase*(multiplier*2)));
			local cnt07 = random(3)+ 2 + math.round((increase*(multiplier*1)));
			...
			if combat == 2 then
				StartCombat(hero,nil,5,55,cnt07,56,cnt07,151,cnt07,t05a,cnt05,t05a,cnt05,nil,"H55_UtopiaWin","/Arenas/CombatArena/FinalCombat/Bank_Utopia.(AdvMapTownCombat).xdb#xpointer(/AdvMapTownCombat)",1);
			end;
			if combat == 3 then
			...
			if combat == 4 then
				StartCombat(hero,nil,5,41,cnt06,42,cnt06,158,cnt06,116,cnt05,116,cnt05,nil,"H55_UtopiaWin","/Arenas/CombatArena/FinalCombat/Bank_Utopia.(AdvMapTownCombat).xdb#xpointer(/AdvMapTownCombat)",1);
			end;				
			if combat == 5 then
...
function H55_GoldReward(hero,level)
	local player = GetObjectOwner(hero);
	local rnd = random(2000)+500;
	local amount = (level*2000)+rnd;
	if level == 4 then amount = amount+3000 end;
	-- if HasHeroSkill(hero,PERK_LUCKY_STRIKE) then
...
function H55_SpellAward(hero,level)
	local player = GetObjectOwner(hero);	
	local rnd5 = random(table.length(H55_SpellsL5))+1;
	local rnd4 = random(table.length(H55_SpellsL4))+1;
	local rnd3 = random(table.length(H55_SpellsL3))+1;
	local rnd2 = random(table.length(H55_SpellsL2))+1;
	local rnd1 = random(table.length(H55_SpellsL1))+1;
	local rnm5 = random(table.length(H55_SpellsL5))+1;
	local rnm4 = random(table.length(H55_SpellsL4))+1;
	local rnm3 = random(table.length(H55_SpellsL3))+1;
	local rnm2 = random(table.length(H55_SpellsL2))+1;
	local rnm1 = random(table.length(H55_SpellsL1))+1;
	local rndchief = random(3);
	local teached = 0;
	local x,y,z = GetObjectPosition(hero);
	if level == 2 then
		if H55_ChieftainsPhonebook[hero] == 1 then
			...
			if KnowHeroSpell(hero,H55_SpellsL5[rnd5]) == nil then
				if H55_RequiredSkill[H55_SpellsL5[rnd5]] == SKILL_LIGHT_MAGIC then
					if HasHeroSkill(hero,SKILL_LIGHT_MAGIC) then
						TeachHeroSpell(hero,H55_SpellsL5[rnd5]);
						if H55_IsThisAIPlayer(player) ~= 1 then H55_ProtectedSignSpell("/Text/Game/Scripts/Banks/Givespell.txt",H55_SpellsTextL5[rnd5],hero,player,H55_MsgSleep,H55_MsgTime);
						Play3DSoundForPlayers(GetPlayerFilter(player), H55_SndSpell,x,y,z) end;
						teached = 1;
					end;
				elseif H55_RequiredSkill[H55_SpellsL5[rnd5]] == SKILL_DARK_MAGIC then
				...
				elseif H55_RequiredSkill[H55_SpellsL5[rnd5]] == SKILL_DESTRUCTIVE_MAGIC then
				...
				elseif H55_RequiredSkill[H55_SpellsL5[rnd5]] == SKILL_SUMMONING_MAGIC then
			...
			elseif KnowHeroSpell(hero,H55_SpellsL4[rnd4]) == nil then
				if H55_RequiredSkill[H55_SpellsL4[rnd4]] == SKILL_LIGHT_MAGIC then
					if HasHeroSkill(hero,SKILL_LIGHT_MAGIC) then
						TeachHeroSpell(hero,H55_SpellsL4[rnd4]);
						if H55_IsThisAIPlayer(player) ~= 1 then H55_ProtectedSignSpell("/Text/Game/Scripts/Banks/Givespell.txt",H55_SpellsTextL4[rnd4],hero,player,H55_MsgSleep,H55_MsgTime);
						Play3DSoundForPlayers(GetPlayerFilter(player), H55_SndSpell,x,y,z) end;
						teached = 1;
					end;
				elseif H55_RequiredSkill[H55_SpellsL4[rnd4]] == SKILL_DARK_MAGIC then
					if HasHeroSkill(hero,SKILL_DARK_MAGIC) then
						TeachHeroSpell(hero,H55_SpellsL4[rnd4]);
						if H55_IsThisAIPlayer(player) ~= 1 then H55_ProtectedSignSpell("/Text/Game/Scripts/Banks/Givespell.txt",H55_SpellsTextL4[rnd4],hero,player,H55_MsgSleep,H55_MsgTime);
						Play3DSoundForPlayers(GetPlayerFilter(player), H55_SndSpell,x,y,z) end;
				...
				elseif H55_RequiredSkill[H55_SpellsL4[rnd4]] == SKILL_DESTRUCTIVE_MAGIC then
					if HasHeroSkill(hero,SKILL_DESTRUCTIVE_MAGIC) then
						TeachHeroSpell(hero,H55_SpellsL4[rnd4]);
						if H55_IsThisAIPlayer(player) ~= 1 then H55_ProtectedSignSpell("/Text/Game/Scripts/Banks/Givespell.txt",H55_SpellsTextL4[rnd4],hero,player,H55_MsgSleep,H55_MsgTime);
						Play3DSoundForPlayers(GetPlayerFilter(player), H55_SndSpell,x,y,z) end;
				...
				elseif H55_RequiredSkill[H55_SpellsL4[rnd4]] == SKILL_SUMMONING_MAGIC then
					if HasHeroSkill(hero,SKILL_SUMMONING_MAGIC) then
						TeachHeroSpell(hero,H55_SpellsL4[rnd4]);
						if H55_IsThisAIPlayer(player) ~= 1 then H55_ProtectedSignSpell("/Text/Game/Scripts/Banks/Givespell.txt",H55_SpellsTextL4[rnd4],hero,player,H55_MsgSleep,H55_MsgTime);
						Play3DSoundForPlayers(GetPlayerFilter(player), H55_SndSpell,x,y,z) end;
			...
			elseif KnowHeroSpell(hero,H55_SpellsL3[rnd3]) == nil then
				if H55_RequiredSkill[H55_SpellsL3[rnd3]] == SKILL_LIGHT_MAGIC then
					if HasHeroSkill(hero,SKILL_LIGHT_MAGIC) then
						TeachHeroSpell(hero,H55_SpellsL3[rnd3]);
						if H55_IsThisAIPlayer(player) ~= 1 then H55_ProtectedSignSpell("/Text/Game/Scripts/Banks/Givespell.txt",H55_SpellsTextL3[rnd3],hero,player,H55_MsgSleep,H55_MsgTime);
						Play3DSoundForPlayers(GetPlayerFilter(player), H55_SndSpell,x,y,z) end;
						teached = 1;
					end;
				elseif H55_RequiredSkill[H55_SpellsL3[rnd3]] == SKILL_DARK_MAGIC then
					if HasHeroSkill(hero,SKILL_DARK_MAGIC) then
						TeachHeroSpell(hero,H55_SpellsL3[rnd3]);
						if H55_IsThisAIPlayer(player) ~= 1 then H55_ProtectedSignSpell("/Text/Game/Scripts/Banks/Givespell.txt",H55_SpellsTextL3[rnd3],hero,player,H55_MsgSleep,H55_MsgTime);
						Play3DSoundForPlayers(GetPlayerFilter(player), H55_SndSpell,x,y,z) end;
				...
				elseif H55_RequiredSkill[H55_SpellsL3[rnd3]] == SKILL_DESTRUCTIVE_MAGIC then
					if HasHeroSkill(hero,SKILL_DESTRUCTIVE_MAGIC) then
						TeachHeroSpell(hero,H55_SpellsL3[rnd3]);
						if H55_IsThisAIPlayer(player) ~= 1 then H55_ProtectedSignSpell("/Text/Game/Scripts/Banks/Givespell.txt",H55_SpellsTextL3[rnd3],hero,player,H55_MsgSleep,H55_MsgTime);
						Play3DSoundForPlayers(GetPlayerFilter(player), H55_SndSpell,x,y,z) end;						
				...
				elseif H55_RequiredSkill[H55_SpellsL3[rnd3]] == SKILL_SUMMONING_MAGIC then
					if HasHeroSkill(hero,SKILL_SUMMONING_MAGIC) then
						TeachHeroSpell(hero,H55_SpellsL3[rnd3]);
						if H55_IsThisAIPlayer(player) ~= 1 then H55_ProtectedSignSpell("/Text/Game/Scripts/Banks/Givespell.txt",H55_SpellsTextL3[rnd3],hero,player,H55_MsgSleep,H55_MsgTime);
						Play3DSoundForPlayers(GetPlayerFilter(player), H55_SndSpell,x,y,z) end;	
				...
				elseif HasHeroSkill(hero,PERK_WISDOM) then
					TeachHeroSpell(hero,H55_SpellsL3[rnd3]);
					if H55_IsThisAIPlayer(player) ~= 1 then H55_ProtectedSignSpell("/Text/Game/Scripts/Banks/Givespell.txt",H55_SpellsTextL3[rnd3],hero,player,H55_MsgSleep,H55_MsgTime);
					Play3DSoundForPlayers(GetPlayerFilter(player), H55_SndSpell,x,y,z) end;					
					teached = 1;
				end;
			elseif KnowHeroSpell(hero,H55_SpellsL5[rnm5]) == nil then
				if H55_RequiredSkill[H55_SpellsL5[rnm5]] == SKILL_LIGHT_MAGIC then
					if HasHeroSkill(hero,SKILL_LIGHT_MAGIC) then
						TeachHeroSpell(hero,H55_SpellsL5[rnm5]);
						if H55_IsThisAIPlayer(player) ~= 1 then H55_ProtectedSignSpell("/Text/Game/Scripts/Banks/Givespell.txt",H55_SpellsTextL5[rnm5],hero,player,H55_MsgSleep,H55_MsgTime);
						Play3DSoundForPlayers(GetPlayerFilter(player), H55_SndSpell,x,y,z) end;
						teached = 1;
					end;
				elseif H55_RequiredSkill[H55_SpellsL5[rnm5]] == SKILL_DARK_MAGIC then
					if HasHeroSkill(hero,SKILL_DARK_MAGIC) then
						TeachHeroSpell(hero,H55_SpellsL5[rnm5]);
						if H55_IsThisAIPlayer(player) ~= 1 then H55_ProtectedSignSpell("/Text/Game/Scripts/Banks/Givespell.txt",H55_SpellsTextL5[rnm5],hero,player,H55_MsgSleep,H55_MsgTime);
						Play3DSoundForPlayers(GetPlayerFilter(player), H55_SndSpell,x,y,z) end;
				...
				elseif H55_RequiredSkill[H55_SpellsL5[rnm5]] == SKILL_DESTRUCTIVE_MAGIC then
					if HasHeroSkill(hero,SKILL_DESTRUCTIVE_MAGIC) then
						TeachHeroSpell(hero,H55_SpellsL5[rnm5]);
						if H55_IsThisAIPlayer(player) ~= 1 then H55_ProtectedSignSpell("/Text/Game/Scripts/Banks/Givespell.txt",H55_SpellsTextL5[rnm5],hero,player,H55_MsgSleep,H55_MsgTime);
						Play3DSoundForPlayers(GetPlayerFilter(player), H55_SndSpell,x,y,z) end;
				...
				elseif H55_RequiredSkill[H55_SpellsL5[rnm5]] == SKILL_SUMMONING_MAGIC then
					if HasHeroSkill(hero,SKILL_SUMMONING_MAGIC) then
						TeachHeroSpell(hero,H55_SpellsL5[rnm5]);
						if H55_IsThisAIPlayer(player) ~= 1 then H55_ProtectedSignSpell("/Text/Game/Scripts/Banks/Givespell.txt",H55_SpellsTextL5[rnm5],hero,player,H55_MsgSleep,H55_MsgTime);
						Play3DSoundForPlayers(GetPlayerFilter(player), H55_SndSpell,x,y,z) end;
			...
			elseif KnowHeroSpell(hero,H55_SpellsL4[rnm4]) == nil then
				if H55_RequiredSkill[H55_SpellsL4[rnm4]] == SKILL_LIGHT_MAGIC then
					if HasHeroSkill(hero,SKILL_LIGHT_MAGIC) then
						TeachHeroSpell(hero,H55_SpellsL4[rnm4]);
						if H55_IsThisAIPlayer(player) ~= 1 then H55_ProtectedSignSpell("/Text/Game/Scripts/Banks/Givespell.txt",H55_SpellsTextL4[rnm4],hero,player,H55_MsgSleep,H55_MsgTime);
						Play3DSoundForPlayers(GetPlayerFilter(player), H55_SndSpell,x,y,z) end;
						teached = 1;
					end;
				elseif H55_RequiredSkill[H55_SpellsL4[rnm4]] == SKILL_DARK_MAGIC then
					if HasHeroSkill(hero,SKILL_DARK_MAGIC) then
						TeachHeroSpell(hero,H55_SpellsL4[rnm4]);
						if H55_IsThisAIPlayer(player) ~= 1 then H55_ProtectedSignSpell("/Text/Game/Scripts/Banks/Givespell.txt",H55_SpellsTextL4[rnm4],hero,player,H55_MsgSleep,H55_MsgTime);
						Play3DSoundForPlayers(GetPlayerFilter(player), H55_SndSpell,x,y,z) end;
				...
				elseif H55_RequiredSkill[H55_SpellsL4[rnm4]] == SKILL_DESTRUCTIVE_MAGIC then
					if HasHeroSkill(hero,SKILL_DESTRUCTIVE_MAGIC) then
						TeachHeroSpell(hero,H55_SpellsL4[rnm4]);
						if H55_IsThisAIPlayer(player) ~= 1 then H55_ProtectedSignSpell("/Text/Game/Scripts/Banks/Givespell.txt",H55_SpellsTextL4[rnm4],hero,player,H55_MsgSleep,H55_MsgTime);
						Play3DSoundForPlayers(GetPlayerFilter(player), H55_SndSpell,x,y,z) end;
				...
				elseif H55_RequiredSkill[H55_SpellsL4[rnm4]] == SKILL_SUMMONING_MAGIC then
					if HasHeroSkill(hero,SKILL_SUMMONING_MAGIC) then
						TeachHeroSpell(hero,H55_SpellsL4[rnm4]);
						if H55_IsThisAIPlayer(player) ~= 1 then H55_ProtectedSignSpell("/Text/Game/Scripts/Banks/Givespell.txt",H55_SpellsTextL4[rnm4],hero,player,H55_MsgSleep,H55_MsgTime);
						Play3DSoundForPlayers(GetPlayerFilter(player), H55_SndSpell,x,y,z) end;
			...
			elseif KnowHeroSpell(hero,H55_SpellsL3[rnm3]) == nil then
				if H55_RequiredSkill[H55_SpellsL3[rnm3]] == SKILL_LIGHT_MAGIC then
					if HasHeroSkill(hero,SKILL_LIGHT_MAGIC) then
						TeachHeroSpell(hero,H55_SpellsL3[rnm3]);
						if H55_IsThisAIPlayer(player) ~= 1 then H55_ProtectedSignSpell("/Text/Game/Scripts/Banks/Givespell.txt",H55_SpellsTextL3[rnm3],hero,player,H55_MsgSleep,H55_MsgTime);
						Play3DSoundForPlayers(GetPlayerFilter(player), H55_SndSpell,x,y,z) end;
						teached = 1;
					end;
				elseif H55_RequiredSkill[H55_SpellsL3[rnm3]] == SKILL_DARK_MAGIC then
					if HasHeroSkill(hero,SKILL_DARK_MAGIC) then
						TeachHeroSpell(hero,H55_SpellsL3[rnm3]);
						if H55_IsThisAIPlayer(player) ~= 1 then H55_ProtectedSignSpell("/Text/Game/Scripts/Banks/Givespell.txt",H55_SpellsTextL3[rnm3],hero,player,H55_MsgSleep,H55_MsgTime);
						Play3DSoundForPlayers(GetPlayerFilter(player), H55_SndSpell,x,y,z) end;
				...
				elseif H55_RequiredSkill[H55_SpellsL3[rnm3]] == SKILL_DESTRUCTIVE_MAGIC then
					if HasHeroSkill(hero,SKILL_DESTRUCTIVE_MAGIC) then
						TeachHeroSpell(hero,H55_SpellsL3[rnm3]);
						if H55_IsThisAIPlayer(player) ~= 1 then H55_ProtectedSignSpell("/Text/Game/Scripts/Banks/Givespell.txt",H55_SpellsTextL3[rnm3],hero,player,H55_MsgSleep,H55_MsgTime);
						Play3DSoundForPlayers(GetPlayerFilter(player), H55_SndSpell,x,y,z) end;						
				...
				elseif H55_RequiredSkill[H55_SpellsL3[rnm3]] == SKILL_SUMMONING_MAGIC then
					if HasHeroSkill(hero,SKILL_SUMMONING_MAGIC) then
						TeachHeroSpell(hero,H55_SpellsL3[rnm3]);
						if H55_IsThisAIPlayer(player) ~= 1 then H55_ProtectedSignSpell("/Text/Game/Scripts/Banks/Givespell.txt",H55_SpellsTextL3[rnm3],hero,player,H55_MsgSleep,H55_MsgTime);
						Play3DSoundForPlayers(GetPlayerFilter(player), H55_SndSpell,x,y,z) end;	
			...
			if teached == 0 then
				if KnowHeroSpell(hero,H55_SpellsL2[rnd2]) == nil then
					...
					if H55_IsThisAIPlayer(player) ~= 1 then H55_ProtectedSignSpell("/Text/Game/Scripts/Banks/Givespell.txt",H55_SpellsTextL2[rnd2],hero,player,H55_MsgSleep,H55_MsgTime);
					Play3DSoundForPlayers(GetPlayerFilter(player), H55_SndSpell,x,y,z) end;						
					teached = 1;
				-- elseif KnowHeroSpell(hero,H55_SpellsL1[rnd1]) == nil then
					-- TeachHeroSpell(hero,H55_SpellsL1[rnd1]);
					-- if H55_IsThisAIPlayer(player) ~= 1 then H55_ProtectedSignSpell("/Text/Game/Scripts/Banks/Givespell.txt",H55_SpellsTextL1[rnd1],hero,player,H55_MsgSleep,H55_MsgTime);
					-- Play3DSoundForPlayers(GetPlayerFilter(player), H55_SndSpell,x,y,z) end;	
					-- teached = 1;
				end;
			end;		
		...
		if H55_ChieftainsPhonebook[hero] == 1 then
			...
			if KnowHeroSpell(hero,H55_SpellsL3[rnd3]) == nil then
				if HasHeroSkill(hero,PERK_WISDOM) then
					...
					Play3DSoundForPlayers(GetPlayerFilter(player), H55_SndSpell,x,y,z) end;				
					teached = 1;
				elseif H55_RequiredSkill[H55_SpellsL3[rnd3]] == SKILL_LIGHT_MAGIC then
					if HasHeroSkill(hero,SKILL_LIGHT_MAGIC) then
						TeachHeroSpell(hero,H55_SpellsL3[rnd3]);
						if H55_IsThisAIPlayer(player) ~= 1 then H55_ProtectedSignSpell("/Text/Game/Scripts/Banks/Givespell.txt",H55_SpellsTextL3[rnd3],hero,player,H55_MsgSleep,H55_MsgTime);
						Play3DSoundForPlayers(GetPlayerFilter(player), H55_SndSpell,x,y,z) end;
			...
			elseif KnowHeroSpell(hero,H55_SpellsL2[rnd2]) == nil then
					TeachHeroSpell(hero,H55_SpellsL2[rnd2]);
					if H55_IsThisAIPlayer(player) ~= 1 then H55_ProtectedSignSpell("/Text/Game/Scripts/Banks/Givespell.txt",H55_SpellsTextL2[rnd2],hero,player,H55_MsgSleep,H55_MsgTime);
					Play3DSoundForPlayers(GetPlayerFilter(player), H55_SndSpell,x,y,z) end;						
					teached = 1;
			...
			elseif KnowHeroSpell(hero,H55_SpellsL5[rnd5]) == nil then
				if H55_RequiredSkill[H55_SpellsL5[rnd5]] == SKILL_LIGHT_MAGIC then
					if HasHeroSkill(hero,SKILL_LIGHT_MAGIC) then
						TeachHeroSpell(hero,H55_SpellsL5[rnd5]);
						if H55_IsThisAIPlayer(player) ~= 1 then H55_ProtectedSignSpell("/Text/Game/Scripts/Banks/Givespell.txt",H55_SpellsTextL5[rnd5],hero,player,H55_MsgSleep,H55_MsgTime);
...
function H55_DocumentAward(hero,level)
	...
	if H55_IsThisAIPlayer(player) ~= 1 then
		if H55_MapType ~= "Campaign" and H55_MapType ~= "Scenario" and H55_NegativeArtifacts == 1 and rndprank >= 98 then
