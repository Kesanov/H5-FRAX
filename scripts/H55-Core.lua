...
H55_DailyEventsCounter = {0,0,0,0,0,0,0,0};
H55_BankDay = 0;
H55_StaticBanks = 0;
...
function H55_ContinuesEvent(player)
	...
	if heroes ~= nil then	
		...
		for i,hero in heroes do
			...
			if H55_WitchesPhonebook[hero] == 1 then
				if strongholdtowns ~= nil then
					for i, town in strongholdtowns do
	...
	while (H55_Switch == 1) do
		...
		for i = 1,8 do
			if (GetPlayerState(i) == 1) and IsPlayerInGhostMode(i) == nil then								
				...
				if (IsPlayerCurrent(i)) and (H55_DailyEventsCounter[i] < H55_Day) then	
